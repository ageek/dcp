...
# empty second-level sub-directory subsubdir1. subdir2 contains a second-level sub-directory subsubdir2
...
def buildfs(input):
    ...
    for f in files:
        ...
        # #print(f)
        cur_node = fs
        for subdir in current_path[:ind]:
            cur_node = cur_node[subdir]
            print(f"cur_node={cur_node} and subdir={subdir}")
        if '.' in f:
            cur_node[f]=True
        else:
            cur_node[f]={}
            
        current_path = current_path[:ind]
